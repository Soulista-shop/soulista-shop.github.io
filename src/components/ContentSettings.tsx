...
export function ContentSettings() {
  ...
  useEffect(() => {
    const fromDb = settings.find((s) => s.section === SITE_BACKGROUND_SECTION)?.text_content;
    if (fromDb) {
      setCustomBackgroundHex(normalizeHex(fromDb));
    }
  }, [settings]);
        ...
        <CardContent className="space-y-6">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end gap-3">
              ...
              <Button
                type="button"
                disabled={saving === "site-background" || !/^#[0-9A-F]{6}$/i.test(customBackgroundHex)}
                onClick={() => void handleSaveSiteBackground(customBackgroundHex)}
                className="min-h-10 sm:mb-0.5"
              >
                <Save className="mr-2 h-4 w-4" />
                {saving === "site-background" ? "Saving…" : "Apply custom"}
              </Button>
            </div>
